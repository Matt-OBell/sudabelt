...
#            contract = contract_obj.browse(contract_id)
    ...
    name = fields.Char(string='Name', required=True, readonly=False, states={'validate':[('readonly', True)]})
    state = fields.Selection(selection=[
                  ...
                  ('validate', 'Approved'), 
                  ('cancel', 'Cancelled')],
                  string='State', readonly=True, help='The state is set to \'Draft\', when a increment request is created.\
                    ...
                    \nThe state is \'Approved\', when increment request is approved by manager.', default='draft')
    user_id = fields.Many2one(related='employee_id.user_id', comodel='res.users', string='User', store=True, default=lambda self:self.env.user)
    date_from = fields.Datetime(string='Create Date', states={'validate':[('readonly', True)]}, default=time.strftime('%Y-%m-01'))
    date_to = fields.Datetime(string='Approved Date', readonly=True)
    last_date = fields.Datetime(string='Last Increment Date', readonly=False, states={'validate':[('readonly', True)]})
    employee_id = fields.Many2one('hr.employee', string='Employee', index=True, required=True, readonly=False, states={'validate':[('readonly', True)]}, help='Leave Manager can let this field empty if this leave request/allocation is for every employee', default=_employee_get)
    manager_id = fields.Many2one('hr.employee', string='Manager', readonly=False, states={'validate':[('readonly', True)]}, help='This area is automatically filled by the user who will approve the request')
    notes = fields.Text(string='Notes', readonly=False, states={'validate':[('readonly', True)]})
#    case_id = fields.Many2one('crm.meeting', string='Meeting', help='Related Meeting for Increment', readonly=False, states={'validate':[('readonly', True)]})
    ...
    department_id = fields.Many2one(related='employee_id.department_id', string='Department', comodel='hr.department', readonly=True, store=True)
    category_id = fields.Many2one('hr.employee.category', string='Category', help='Category of Employee', readonly=False, states={'validate':[('readonly', True)]})
    contract_id = fields.Many2one('hr.contract', 'Contract', states={'validate':[('readonly', True)]}, index=True)
    old_salary = fields.Float(string='Current Wage', states={'validate':[('readonly', True)]}, help='Basic Pay of employee')
    expected_salary = fields.Float(string='Expected Wage', readonly=False, states={'validate':[('readonly', True)]}, help='Expected Basic Pay of employee')
    new_salary = fields.Float(string='New Salary', help='New Basic Pay/Salary Approved by Management', readonly=False, states={'validate':[('readonly', True)]})
    company_id = fields.Many2one('res.company', string='Company', required=False, readonly=False, states={'validate':[('readonly', True)]}, default=lambda self:self.env['res.company']._company_default_get('salary.increment'))
    ...
    def set_to_draft(self):
        ...
        history_ids = self.env['contract.history'].search([('increment_id', '=', self.id)])
        history_ids.unlink()
    ...
    def salary_validate(self):
        for sal in self:
            ...
            vals = {'wage':sal.old_salary, 'employee_id': sal.employee_id.id, 'revision_date': time.strftime('%Y-%m-%d'), 'increment_id': sal.id, 'name': 'Increment', 'contract_id':sal.contract_id.id, 'struct_id': sal.contract_id.struct_id.id}
            self.env['contract.history'].create(vals)
            sal.contract_id.write({'wage': sal.new_salary})
        return self.write({'state':'validate', 'manager_id': manager.id, 'date_to': time.strftime('%Y-%m-%d')})
